...
int main(int argc, char** argv){
    ...
    declarar_variable(fasm, "n", ENTERO, 1);
    ...
    //IMPRIMO 1 y LUEGO REALIZO LAS OPERACIONES DEL IF ANIDADO
    ...
    escribir_operando(fasm,"m",1);
    escribir_operando(fasm,"n",1);
    restar(fasm,1,1);

    escribir_operando(fasm,"0",0);
    menor(fasm,0,0,etiqueta);


   

    ifthen_inicio(fasm, 0, etiqueta); //PRIMER IF THEN ELSE

    escribir_operando(fasm,"0",0);
    escribir(fasm, 0, ENTERO);

    etiqueta = etiquetas[cima_etiquetas];
    ifthen_fin(fasm, etiqueta);
    cima_etiquetas--;
